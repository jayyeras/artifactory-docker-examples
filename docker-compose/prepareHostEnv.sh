...
setPermissions () {
    # Fix directories permissions only on Linux
    if [ ${OS_NAME} == "Linux" ]; then
        echo "Creating ${ARTIFACTORY_USER_NAME} user"
        if id ${ARTIFACTORY_USER_NAME} 2> /dev/null; then
            echo "User ${ARTIFACTORY_USER_NAME} already exists"
        else
        ...
        chown -R ${ARTIFACTORY_USER_NAME}. ${ROOT_DATA_DIR}/artifactory || errorExit "Setting ownership of ${ROOT_DATA_DIR}/artifactory to ${ARTIFACTORY_USER_ID} failed"
        chmod -R 777 ${ROOT_DATA_DIR}/nginx || errorExit "Setting permissions 777 on ${ROOT_DATA_DIR}/nginx failed"
    fi
}
